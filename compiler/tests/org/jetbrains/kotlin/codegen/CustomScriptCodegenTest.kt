...
class CustomScriptCodegenTest : CodegenTestCase() {
    ...
    @Ignore("WIP")
    fun testStandardDefinition() {
        createScriptTestEnvironment("org.jetbrains.kotlin.codegen.TestScriptWithSimpleEnvVars")
        loadScript("val x = 1")
        val res = generateScriptClass()
        assertNull(res.getAnnotation(KotlinScript::class.java))
    }
